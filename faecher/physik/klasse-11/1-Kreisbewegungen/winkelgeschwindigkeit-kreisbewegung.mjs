...
function formatElapsed(value) {
  return formatNumber(Math.floor((value + Number.EPSILON) * 10) / 10, 1);
}

function formatRadians(value) {
  const known = [[Math.PI / 6, "π/6"], [Math.PI / 4, "π/4"], [Math.PI / 3, "π/3"], [Math.PI / 2, "π/2"], [Math.PI * 2 / 3, "2π/3"], [Math.PI, "π"], [Math.PI * 1.5, "3π/2"], [Math.PI * 5 / 3, "5π/3"], [Math.PI * 2, "2π"]];
  return known.find(([number]) => Math.abs(number - value) < 0.000001)?.[1] || formatNumber(value);
}
...
function setupSpeedSimulation() {
  ...
  function update() {
    ...
    const vectorLabel = host.querySelector("[data-speed-vector-label]");
    const labelPoint = {
      x: clamp(tangent.x - vectors.inward.x * 42, 30, 410),
      y: clamp(tangent.y - vectors.inward.y * 42, 20, 420),
    };
    vectorLabel.setAttribute("transform", `translate(${labelPoint.x} ${labelPoint.y})`);
...
function setupFormulaBuilder({ id, expected, equation, rememberId }) {
  ...
  const render = () => {
    ...
    reset.addEventListener("click", () => { slots.forEach((slot) => { slot.value = ""; }); selected = ""; values = shuffleIncorrect(expected, expected); document.getElementById(rememberId).hidden = true; render(); });
...
function setupFrequency() {
  ...
  const panel = input.closest("[data-physics-panel]");
  const body = panel.querySelector("[data-frequency-body]");
  const radius = panel.querySelector("[data-frequency-radius]");
  const status = panel.querySelector("[data-frequency-status]");
  let elapsedSeconds = 0;
  let running = false;
  let frame = 0;
  let last = 0;
  ...
  function update() {
    ...
    const currentElapsed = elapsedInCycle(elapsedSeconds, period);
    const angle = angleAtCycleElapsed(elapsedSeconds, period) - Math.PI / 2;
    const vectors = circleVectors(angle, 96);
    const point = { x: 140 + vectors.position.x, y: 140 + vectors.position.y };
    body.setAttribute("cx", point.x);
    body.setAttribute("cy", point.y);
    radius.setAttribute("x2", point.x);
    radius.setAttribute("y2", point.y);
    panel.querySelector("[data-period-value]").textContent = formatNumber(period, 1);
    panel.querySelector("[data-frequency-value]").textContent = formatNumber(frequencyFromPeriod(period));
    panel.querySelector("[data-frequency-elapsed]").textContent = formatElapsed(currentElapsed);
  }
  ...
  function start() {
    if (window.matchMedia("(prefers-reduced-motion: reduce)").matches) {
      status.textContent = "Bewegung ist wegen der Einstellung für reduzierte Bewegung angehalten.";
      return;
    }
    if (!running) {
      running = true;
      last = 0;
      status.textContent = "Bewegung läuft. Die Zeitanzeige beginnt nach jeder Runde wieder bei 0.";
      frame = requestAnimationFrame(tick);
    }
  }

  function pause(message = "Bewegung pausiert.") {
    running = false;
    cancelAnimationFrame(frame);
    status.textContent = message;
  }

  panel.querySelector('[data-frequency-action="start"]').addEventListener("click", start);
  panel.querySelector('[data-frequency-action="pause"]').addEventListener("click", () => pause());
  panel.querySelector('[data-frequency-action="reset"]').addEventListener("click", () => { pause("Bewegung zurückgesetzt."); elapsedSeconds = 0; update(); });
  input.addEventListener("input", () => { pause("Umlaufdauer geändert. Starte die Bewegung erneut."); elapsedSeconds = 0; update(); });
  document.addEventListener("visibilitychange", () => { if (document.hidden && running) pause("Bewegung pausiert, weil die Seite nicht sichtbar ist."); });
  new MutationObserver(() => { if (panel.hidden && running) pause("Bewegung pausiert, weil der Reiter gewechselt wurde."); }).observe(panel, { attributes: true, attributeFilter: ["hidden"] });
  update();
}

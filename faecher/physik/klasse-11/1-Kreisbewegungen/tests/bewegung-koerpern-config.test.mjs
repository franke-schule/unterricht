...
assert.match(html, /a = 2,5&nbsp;<span class="physics-fraction" role="img" aria-label="Meter pro Sekunde zum Quadrat">/);
...
assert.match(html, /<select id="acceleration-unit" aria-label="Einheit der berechneten Geschwindigkeit"><option value="">Einheit<\/option>/);
assert.doesNotMatch(html, /class="unit-choice"/);
assert.match(source, /const unitCorrect = unit === "m\/s" \|\| unit === "km\/h";/);
...
assert.match(source, /class="chart-axis-label" x="7" y="16">v in</);
assert.match(source, /class="svg-fraction" role="img" aria-label="Meter pro Sekunde"/);
assert.match(source, /class="svg-fraction-bar" x1="8" y1="38" x2="26" y2="38"/);
assert.match(source, /class="chart-value-label" text-anchor="end" x="48" y="135">15</);
assert.match(source, /class="chart-value-label" text-anchor="end" x="48" y="75">25</);
assert.doesNotMatch(source, /x="190" y="122">15|x="440" y="62">25/);
...
assert.match(html, /href="sicherungsblatt-aufgabe-1-loesungen\.pdf\?v=20260911a" download hidden/);

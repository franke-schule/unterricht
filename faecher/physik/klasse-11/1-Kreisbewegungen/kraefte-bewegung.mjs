...
function setupAccelerationCalculation() {
  document.getElementById("check-force-acceleration").addEventListener("click", () => {
    const input = document.getElementById("force-acceleration-answer"); const value = numberValue(input.value); const digits = significantDigitCount(input.value); const unit = unitChoiceValue("force-acceleration-unit"); const valueCorrect = Number.isFinite(value) && Math.abs(value - 12) <= 0.01; const unitCorrect = unit === "m/s²";

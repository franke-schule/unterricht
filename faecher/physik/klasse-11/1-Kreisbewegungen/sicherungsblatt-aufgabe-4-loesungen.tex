...
\begin{tabularx}{\linewidth}{@{}>{\bfseries}p{27mm} X >{\centering\arraybackslash}p{20mm}@{}}
\toprule[0.4pt]
...
\begin{tikzpicture}[x=1mm,y=1mm,>=Latex]
  ...
  \node[font=\scriptsize] at (7,0) {$\varphi$};
\end{tikzpicture}
\end{minipage}\hfill
...
\small
\rowcolors{2}{Sky}{white}
\begin{tabularx}{\linewidth}{@{}>{\bfseries}p{40mm} X@{}}
\toprule[0.4pt]
...
Gesucht: & $F_{\mathrm Z}$ \\
Ansatz: & $F_{\mathrm Z}=\dfrac{m\cdot v_{\mathrm B}^{2}}{r}$ \\
...
Gesucht: & $v_{\mathrm B}$ \\
Ansatz: & $v_{\mathrm B}=\sqrt{\dfrac{F_{\mathrm Z}\cdot r}{m}}$ \\

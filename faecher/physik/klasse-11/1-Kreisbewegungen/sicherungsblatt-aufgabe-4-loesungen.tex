...
\begin{tikzpicture}[x=1mm,y=1mm,>=Latex]
  ...
  \node[font=\scriptsize] at (4.2,0) {$\alpha$};
\end{tikzpicture}
\end{minipage}\hfill
\begin{minipage}[t]{0.74\linewidth}
\vspace{0pt}\small
\begin{tabularx}{\linewidth}{@{}>{\bfseries}p{27mm}X@{}}
Grundgesetz: & $F\cdot\Delta t=m\cdot\Delta v$ \\[0.9mm]
\mbox{Ähnliche Dreiecke:} & $\dfrac{\Delta v}{\Delta s}=\dfrac{v_{\mathrm B}}{r}$ (beide gleichschenklig, gleicher Winkel $\alpha$, SWS) \\[0.9mm]
Näherung: & (kleines $\Delta t$) \quad $\Delta s\approx\Delta x=v_{\mathrm B}\cdot\Delta t \;\Rightarrow\; \Delta v=\dfrac{v_{\mathrm B}^{2}\cdot\Delta t}{r}$ \\[0.9mm]
Einsetzen: & $F_{\mathrm Z}=\dfrac{m\cdot v_{\mathrm B}^{2}}{r}$, mit $v_{\mathrm B}=\omega\cdot r$: $F_{\mathrm Z}=m\cdot\omega^{2}\cdot r$
\end{tabularx}
\end{minipage}
...
\footnotesize
\rowcolors{2}{Sky}{white}
\begin{tabularx}{\linewidth}{@{}>{\bfseries}p{47mm} X@{}}
\toprule[0.4pt]
Situation & Wirkende Kraft \\
\midrule[0.3pt]
Mond auf seiner Bahn um die Erde & Gravitationskraft \\
Hammerwurf (von oben betrachtet) & Seilkraft \\
Auto in einer flachen Kurve & Haftreibungskraft \\
Elektron um den Atomkern (Bohr) & elektrische Kraft \\
Wäsche in der Schleudertrommel & Normalkraft der Trommelwand \\
Kettenkarussell & Kettenkraft und Gewichtskraft zusammen \\
\bottomrule[0.4pt]
\end{tabularx}
\vspace{1mm}
\colorbox{Gold}{\parbox{\dimexpr\linewidth-2\fboxsep\relax}{\small\centering\bfseries Leitfrage: Welche Kraft wirkt hier als Zentripetalkraft? \\ Nur eine einzige Kraft zum Mittelpunkt $\to$ gleichsetzen mit $F_{\mathrm Z}$ · Kraft zu klein $\to$ Bahnradius wird größer}}
...
\end{tabularx}}
\vspace{0.3mm}\hfill\textcolor{Blue}{\bfseries Ergebnis: $\approx 18\unitfrac{m}{s}$ \quad ($17{,}89\unitfrac{m}{s}\cdot 3{,}6\approx 64\unitfrac{km}{h}$) \quad 2 gültige Ziffern}
\end{tcolorbox}}

...
function setupFinalQuiz() {
  const items = [
    ...
    { question: "Welche Formel beschreibt die maximale Bahngeschwindigkeit auf einer ebenen Kurve?", correct: ["correct"], options: [["correct", "{{v_B,max}} = {{sqrt:μ · g · r}}"], ["linear", "{{v_B,max}} = μ · g · r"], ["mass", "{{v_B,max}} = {{sqrt:m · μ · g · r}}"], ["denominator", "{{v_B,max}} = {{sqrt:μ · g|r}}"]], feedback: "Die Masse kürzt sich; nach dem Umstellen wird die Quadratwurzel aus μ · g · r gezogen.", hint: "Beginne mit m · {{v_B²|r}} = μ · m · g." },

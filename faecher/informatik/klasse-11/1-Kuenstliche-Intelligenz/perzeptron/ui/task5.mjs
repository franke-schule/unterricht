...
const DECISION_ORDER = ['product1','product2','sum','comparison','output','class'];
function fieldFeedback(id, kind, beginning, field) { feedback(id, kind, beginning + ' <strong>' + (FIELD_LABELS[field] || field) + '</strong>.', true); }
function setupDecision() { document.querySelector('#check-decision').addEventListener('click', () => { const result = checkFields(document.querySelector('#decision-form'), {product1:-4,product2:2,sum:-2,comparison:'lt',output:'0',class:'danger'}); const solved = result.correct === result.total; const decisionText = {sum:solved ? 'a = −2' : 'a = ?', comparison:solved ? '−2 < 3' : 'a ? θ', output:solved ? '0' : '?', class:solved ? 'gefährlich' : 'noch unbekannt'}; Object.entries(decisionText).forEach(([part,text]) => document.querySelectorAll('[data-decision=' + part + ']').forEach((element) => { element.textContent = text; })); if (solved) feedback('decision-feedback','success','Korrekt. Die Summe ist −2; das Perzeptron gibt 0 aus und klassifiziert den Hai als gefährlich.'); else if (result.correct) fieldFeedback('decision-feedback','partial',result.correct + ' von ' + result.total + ' Einträgen stimmen. Prüfe zuerst Schritt ' + (DECISION_ORDER.indexOf(result.wrong[0]) + 1) + ':',result.wrong[0]); else feedback('decision-feedback','error','Noch nicht korrekt. Gehe die Schritte der Reihe nach durch: Berechne zuerst die Ergebnisse in Schritt 1 und 2 und addiere sie erst in Schritt 3.'); }); }
